...
public class StdLibBenchmark {
    ...
    public static class ContainsState {
        public Collection<Integer> staticSample;
        ...
        @Setup(Level.Trial)
        public void init(final Main.StdLibValues values) {
            this.collection = values.newCollection();

            this.staticSample = new ArrayList<>(values.collectionSize());
            final ThreadLocalRandom random = ThreadLocalRandom.current();
            for (int index = 0; index < values.collectionSize(); index++) {
                this.staticSample.add(random.nextInt(Integer.MIN_VALUE, 0));
            }
